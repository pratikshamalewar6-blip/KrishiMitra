"""
KrishiMitra - Crop Disease Classifier Training Pipeline (Optimized for RTX 3050)

Trains EfficientNet-B0 on crop disease datasets (PlantVillage) with AMP & GradScaler.
...
def train_epoch(
    ...
    """
    Runs a single training epoch using Automatic Mixed Precision (AMP) for RTX 3050 VRAM optimization.
    """
    model.train()
    running_loss = 0.0
    correct = 0
    ...
    for images, labels in tqdm(loader, desc="Training Batches", leave=False):
        ...
        # Backward pass with GradScaler for stability under FP16
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train EfficientNet-B0 Disease Classifier with AMP")
    ...
    logger.info("Initializing Optimized Classification Training Pipeline...")
    ...
    try:
        ...
        logger.info("Please make sure split files/folders are correctly structured.")
        raise e
    ...
    # 3. Criterion, Optimizer, Scheduler & GradScaler for AMP
    ...
    # GradScaler for mixed precision training on RTX 3050
    scaler = torch.amp.GradScaler("cuda" if "cuda" in config.DEVICE else "cpu", enabled=("cuda" in config.DEVICE))
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        # Train and validate with AMP scaler
